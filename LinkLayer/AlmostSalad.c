...
void gpio_state_change_callback(int pi, unsigned gpio, unsigned level, uint32_t tick) {
	...
	if (tick - last_tick > 120000) {
	 bit_count = 0;
        ...
        last_tick = tick;
    } else if (bit_count == 2 && level == 1 && tick-last_tick > 40000 && tick-last_tick < 115000) {
	...
	if (tick - last_tick > 120000) {
	 bit_count = 0;
        ...
        last_tick = tick;
    } else if (bit_count == 3 && level == 0 && tick-last_tick > 40000 && tick-last_tick < 115000) {
    ...
    } else if (bit_count == 4 && level == 1 && tick-last_tick > 40000 && tick-last_tick < 115000) {
