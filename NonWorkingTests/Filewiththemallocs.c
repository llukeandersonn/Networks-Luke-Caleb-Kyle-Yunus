...
int main(int argc, char *argv[]) {  
    ...
    while(1){
    char* buffPtr = malloc(sizeof(char) * MAX_SIZE); 
    char* rcvPtr = malloc(sizeof(char) * MAX_SIZE); 
    int* binPtr = malloc(sizeof(int) * MAX_SIZE * 8 + 1)
    printf("Enter a string...\n");
    fgets(buffPtr, MAX_SIZE + 1, stdin);
    printf("string is: %s", buffPtr);
    for (k = 0; k < MAX_SIZE*8; k++) {
        binPtr[k] = 0;
    }
    printf("Binary is: ");
    strToBin(binPtr, MAX_SIZE*8, MAX_SIZE, &buffPtr);
    ...
    */
    ...
    int sObin = sizeof(binPtr) / sizeof(binPtr[0]);
    time_sleep(2);
    int *bin_arr = binPtr;
