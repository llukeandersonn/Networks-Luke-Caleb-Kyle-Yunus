...
void *oursend(void* i) {
    ...
    int sOaddy = sizeof(sdata->out_address) / sizeof(sdata->out_address[0]);
...
void gpio_state_change_callback(int pi, unsigned gpio, unsigned level, uint32_t tick) {
   ...
   //     printf("Average read rate: %u microseconds\n", avg_read_rate);
        ...
        if (bit_count == 12){
            int right_addy = bits_to_len(in_address);
	printf("Right ADDY: %d \n Our ADDY: %d", right_addy, *address);		
        ...
        if(bit_count>=16 && checkLast16Zeroes(bits, bit_count-13)){
           if(!forward){
                ...
                int sObits = sizeof(bits) / sizeof(bits[0]);
                fdata->pi_send = pi;
                fdata->size = sObits;
                fdata->out_address = in_address;
                fdata->msg = bits;
    
    if (pthread_create(&sender_thread, NULL, oursend,(void *) fdata)) {
...
int main(int argc, char *argv[]) {  
    ...
    int input_address_size = getline(&address, &addsize, stdin);
    //printf("%d", input_address_size);
    if(input_address_size != 2){
   ...
   pthread_t listener_thread;
    ...
    int output_address_size = getline(&out_int, &out_size, stdin);
    if(output_address_size != 2){
        printf("Invalid address length.\n");
        return 1;      
    }
