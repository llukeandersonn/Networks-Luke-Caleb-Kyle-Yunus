...
void gpio_callback(int pi, int gpio, int level, uint32_t tick) {
    // Ignore transitions that are too close together
    static uint32_t last_tick = 0;
    if (tick - last_tick < 100) { // Adjust this threshold as needed
        return;
    }
    last_tick = tick;

    if (level == current_level) {
        return; // Ignore repeated levels
    }

    // Manchester decoding logic
    if (level == 1 && current_level == 0) { 
        // Rising edge
        decoded_byte |= (1 << (7 - bit_count));
    }
...
{  
    ...
    //set_pull_up_down(pi, 26, PI_PUD_DOWN);
    callback(pi, 26, EITHER_EDGE, &gpio_callback);
    /*regularize();
   time_sleep(0.5);
