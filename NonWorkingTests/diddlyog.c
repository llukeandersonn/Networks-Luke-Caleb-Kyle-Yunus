...
int main(int argc, char *argv[]) {
    ...
    // Using getline for dynamic user input
    char *buf = NULL;  // Pointer for getline
    size_t bufsize = 0;  // Tracks the size of the input buffer
    ssize_t input_size;  // Size of the user input
    ...
    while (1) {
        ...
        buf[input_size - 1] = '\0';  // Null-terminate the string by removing the newline

        // Dynamically allocate bits array based on the input length
        int bit_length = input_size * 8;  // Each character is 8 bits
        bits = malloc(bit_length * sizeof(int));  // Dynamically allocate for the bits array
        if (bits == NULL) {
            fprintf(stderr, "Memory allocation failed for bits\n");
            free(buf);  // Free memory before exiting
            return 1;
        }

        // Dynamically allocate binary array
        int *bin = malloc(bit_length * sizeof(int));  // Dynamically allocate binary array
        if (bin == NULL) {
            fprintf(stderr, "Memory allocation failed for bin\n");
            free(buf);
            free(bits);  // Free memory before exiting
            return 1;
        }

        // Convert string to binary
        printf("string is: %s\n", buf);
        strToBin(bin, bit_length, input_size, buf);  // Convert input to binary
        ...
        // Add parity bit for error checking
        add_parity_bit(bin, bit_length);

        // Send the Manchester-encoded header and message
        int head[] = {1, 0, 1, 0, 1};  // Header for synchronization
        int sOhead = sizeof(head) / sizeof(head[0]);
        sendFunction(head, sOhead);
        sendFunction(bin, bit_length);

        free(bin);  // Free binary array

        time_sleep(5);  // Delay for the listener to process

        // Dynamically allocate recv array to hold the decoded message
        char *recv = malloc(input_size * sizeof(char));  // Dynamically allocate recv
        if (recv == NULL) {
            fprintf(stderr, "Memory allocation failed for recv\n");
            free(buf);
            free(bits);  // Free memory before exiting
            return 1;
        }

        // Simulate or process received bits
        printf("\nReceived bits: ");
        ...
        // Convert received bits back to string
        binToStr(bits, recv, input_size);  // Convert bits back to string
        printf("Received message: %s\n", recv);

        // Free recv and bits memory
        free(recv);
        free(bits);
        bit_count = 0;  // Reset bit count for next iteration
    }

    // Free dynamically allocated buf after loop ends
    free(buf);
